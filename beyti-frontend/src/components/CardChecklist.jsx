...
const CardChecklist = ({
  ...
  return (
    <div className={`bg-grey-200 shadow-soft-lift rounded-lg p-6 ${className}`} {...props}>
      {/* Title */}
      {title && (
        <>
          <h2 className="text-card-h2 text-charcoal-600 mb-2">{title}</h2>
          <div className="border-b border-grey-stroke mb-4"></div>
        </>
      )}

      {/* Subheading */}
      {subheading && (
        <h3 className="text-light-h3 text-charcoal-600 mb-3">{subheading}</h3>
      )}
      ...
      <div className="space-y-2">
        {items.map((item, index) => (
          <div key={index} className="flex items-center gap-3">
            {/* Icon */}
            {item.completed ? (
              <CheckCircle size={20} weight="fill" className="text-sage-500 flex-shrink-0" />
            ) : (
              <Circle size={20} weight="regular" className="text-sage-500 flex-shrink-0" />
            )}
